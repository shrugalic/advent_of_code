...
fn adapter_combination_count(numbers: &[String]) -> usize {
    let numbers = add_start_and_end_then_sort(numbers);
    let diffs = diffs_between(&numbers);
...
#[cfg(test)]
mod tests {
    use crate::{adapter_combination_count, product_of_joltage_diff_1_and_3_counts};
...
3";
    ...
    fn part2() {
        assert_eq!(
            adapter_combination_count(&read_file_to_lines("input.txt")),
            6908379398144
        );
    }
}
