...
/// For example: 123 ORE
const INGREDIENT_PATTERN: &'static str = r"^((\d+) (\D+)(, )?)+$";
...
impl NanoFactory {
    fn with_formula(formula: Formula) -> Self {
        NanoFactory::with_formulae(&[formula])
    }
    fn with_formulae(formulae: &[Formula]) -> Self {
        NanoFactory {
            formulae: formulae.iter().cloned().collect(),
        }
    }
...
#[cfg(test)]
mod tests {
    use crate::{Chemical, Formula, NanoFactory};
    ...
    fn single_ingredient_factory() {
        ...
        assert_eq!(
            nf,
            NanoFactory::with_formula(Formula::new(vec![Chemical::new(1, "ORE")], target))
        );
    }
    #[test]
    fn dual_ingredient_factory() {
        ...
        assert_eq!(
            nf,
            NanoFactory::with_formula(Formula::new(
