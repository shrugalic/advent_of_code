...
pub(crate) fn count_entries_with_required_fields(lines: &[String]) -> usize {
    count_valid_entries(lines, false)
}

pub(crate) fn count_entries_with_required_fields_and_valid_values(lines: &[String]) -> usize {
...
fn is_entry_valid(entry: &str, check_values: bool) -> bool {
    // println!("{:?}", entry);
...
pid:3556412378 byr:2007"
    ...
    fn part2() {
        assert_eq!(
            count_entries_with_required_fields_and_valid_values(&read_file_to_lines(
                "input/day04.txt"
            )),
            160
        );
