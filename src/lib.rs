...
fn distance_of_closest_intersection(path1: &str, path2: &str) -> usize {
    ...
    return point_closest_to_origin(candidates)
        .unwrap()
        .manhattan_distance();
}

fn point_closest_to_origin(mut candidates: Vec<Point>) -> Option<Point> {
    candidates.sort_by(|a, b| a.manhattan_distance().cmp(&(b.manhattan_distance())));
...
impl Direction {
    ...
    fn orthogonal(&self, other: &Direction) -> bool {
        return self.vertical() && other.horizontal() || self.horizontal() && other.vertical();
    }
}
...
#[cfg(test)]
mod tests {
    use crate::{distance_of_closest_intersection, path_to_lines, Direction::*, Line, Point};
